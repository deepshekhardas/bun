...
static size_t RareAnsiSequenceEnd(const uint8_t* input, size_t len, size_t pos)
{
    if (pos + 1 >= len)
...
// given that chunk's lane bitmasks. Returns how many input bytes the chunk
// consumed (may exceed the vector width when a sequence straddles the end) and
// writes the chunk's visible columns to `countDelta`.
//
// Pure scalar, and deliberately outside the per-target region: inlining it into
// the no-escape loop costs that loop roughly 40% of its throughput, and one copy
// per SIMD target would be five near-identical functions full of post-baseline
// instructions for verify-baseline-static to allowlist.
static size_t ProcessEscapeChunk(const uint8_t* input, size_t len, size_t i, size_t N,
    uint64_t esc, uint64_t prn, uint64_t fin, uint64_t term, AnsiExcludeState& state, size_t& countDelta)
{
...
size_t VisibleLatin1WidthExcludeANSIImpl(const uint8_t* HWY_RESTRICT input, size_t len)
{
    ...
    while (i + N <= len) {
        ...
        // Cold: classify the bytes the escape scanner needs, then hand the
        // chunk's lane bitmasks to the scalar half.
        ...
        size_t countDelta = 0;
        i += ProcessEscapeChunk(input, len, i, N, maskToBits(esc_m), maskToBits(printable_m),
            maskToBits(final_m), maskToBits(term_m), state, countDelta);
        count += countDelta;
    }
